...
four genuine issues — evidence the system works, not toy checks:
...
4. **€850,000 Loamin contingent consideration flagged as performance-linked.**
   Non-cash on completion, so liquidity metrics are computed both including
   and excluding it.
...
- **Senus 2030 gap:** the ≥50% CAGR commitment implies FY26 revenue of
  ...
  **+81.6% vs H2 FY25**. The dashboard shows this as the board's #1 question.
- **Cash runway ~10.8 months** (€735.2k cash ÷ H1 operating burn of
...
- Frontend: https://senus-dashboard.onrender.com
- Backend: https://senus-board-report-gx3v.onrender.com
...
1. **Backend `CORS_ORIGINS`** — set this env var on the backend service to
   ...
   ```
   CORS_ORIGINS=https://senus-dashboard.onrender.com
   ```
...
2. **Frontend `VITE_API_URL`** — set this build-time env var on the
   ...
   ```
   VITE_API_URL=https://senus-board-report-gx3v.onrender.com
   ```
